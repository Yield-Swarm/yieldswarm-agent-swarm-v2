...
Infrastructure in this repo: `deploy/deploy-swarm-monolith.yaml`, `deploy/akash-odysseus.sdl.yml`,
`deploy/terraform/`, `scripts/akash-deploy.sh`, Vercel auto-deploy from `main`/`development`.
...
| `crypto.SOL.address` | Treasury ops wallet | Crypto record | Wallet resolution |
| `crypto.TON.address` | TON treasury | Crypto record | Wallet resolution |
| `kairo.<domain>` | Vercel Kairo app (`/kairo`) | CNAME | Driver marketplace |
