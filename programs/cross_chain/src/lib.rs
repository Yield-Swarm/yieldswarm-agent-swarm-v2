...
pub mod cross_chain {
    ...
    /// Register a Mining Root destination for Helix yield routing (IoTeX, Base, etc.).
    pub fn register_mining_root(
        ctx: Context<RegisterMiningRoot>,
        root_key: [u8; 32],
        chain_id: u32,
        ...
        require!(
            weight_bps <= state::MiningRoot::MAX_WEIGHT_BPS,
            CrossChainError::InvalidMiningRootWeight
        );
        require!(chain_id != 0, CrossChainError::InvalidOriginChain);

        let root = &mut ctx.accounts.mining_root;
        root.authority = ctx.accounts.authority.key();
        root.root_key = root_key;
        root.chain_id = chain_id;
        root.destination_hash = destination_hash;
        root.weight_bps = weight_bps;
        root.total_routed = 0;
        root.bump = ctx.bumps.mining_root;

        emit!(MiningRootRegistered {
            root_key,
            chain_id,
            weight_bps,
        });
    ...
    /// Route harvested yield to a registered Mining Root (off-chain settlement anchor).
    pub fn route_yield_to_root(
        ctx: Context<RouteYieldToRoot>,
        amount: u64,
        zk_batch_root: [u8; 32],
    ) -> Result<()> {
        ctx.accounts.bridge_state.assert_operational()?;
        require!(amount > 0, CrossChainError::BelowMinimum);
        ...
        emit!(YieldRoutedToRoot {
            root_key: root.root_key,
            chain_id: root.chain_id,
            amount,
            zk_batch_root,
            total_routed: root.total_routed,
        });
...
#[derive(Accounts)]
#[instruction(root_key: [u8; 32])]
pub struct RegisterMiningRoot<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
    ...
    #[account(
        init,
        payer = authority,
        space = 8 + MiningRoot::INIT_SPACE,
        seeds = [MiningRoot::SEED, &root_key],
        bump,
    )]
...
#[derive(Accounts)]
pub struct RouteYieldToRoot<'info> {
    pub authority: Signer<'info>,
    ...
    #[account(
        mut,
        seeds = [MiningRoot::SEED, mining_root.root_key.as_ref()],
