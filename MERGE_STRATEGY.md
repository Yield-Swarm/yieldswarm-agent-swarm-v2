...
> Updated June 15, 2026 — **Merge complete on `main`**
> Repo: `yieldswarm-agent-swarm-v2`  
> Integration commit: `3691a0e` (18 canonical `cursor/*` branches merged)

## Current State (Post-Merge)

| Branch | Commit | Status |
|--------|--------|--------|
| `main` | `3691a0e` | ✅ Full monorepo (~545 files) |
| `development` | `3691a0e` | Synced with main |
| `testnet` | `3691a0e` | Synced with main |
| `devnets` | `3691a0e` | Synced with main |
| `production` | `3691a0e` | Synced with main |
| `MAINNET` | `3691a0e` | Synced with main |

**54 `cursor/*` branches** remain on GitHub for audit. 25 duplicate Vault branches
should be closed without merge. Canonical winners are documented below.

### Recommended next actions

```bash
# 1. Enable branch protection on main, testnet, production, MAINNET
# 2. Close duplicate Vault PRs (see "Branches to Skip" below)
# 3. Verify builds:
cd frontend && npm install && npm run build
npm install && npm run build          # payments + Kairo app
cd backend && npm test
python3 -m pytest tests/ 2>/dev/null || true
# 4. Feature work branches off development:
git checkout development
git checkout -b cursor/<feature>-eb09
```
...
Kairo (driver-first marketplace) lives in this monorepo and shares:

- `src/lib/kairo/` — cryptographic identity, signed telemetry, Mandelbrot routing
- `src/app/api/kairo/` — driver registration, telemetry ingest, earnings API
- `src/app/kairo/dashboard` — contribution + rewards dashboard
- `kairo/services/mandelbrot_ingest.py` — Python bridge to Odysseus ChromaDB
- `frontend/src/wallet` — unified wallet layer
- `src/lib/payments` — Square, Wise, Web3 (1% customer fee, 2× driver pay)
- Vault secret injection patterns

New Kairo work: branch off `development` with `cursor/kairo-*` prefix.
